...
class PlcMonitorEngine:
    ...
    def __init__(self):
        ...
        # Churn ("real sim") test state
        self._churn_running = False
        self._churn_done = False
        self._churn_result: dict | None = None
        self._churn_progress: dict = {}
    ...
    def subscribe(self, var_names: list[str], force: bool = False) -> dict:
        """Subscribe to variables by name. Sends SET command to PLC.
        Always includes CPU profiler variables.

        Enforces a safety cap derived from the PLC cycle time (see
        get_subscription_limits). Pass force=True to override.
        """
        var_ids = []
        not_found = []
        ...
        err = self._check_sub_size(len(merged), force)
        if err is not None:
            return err
    ...
    def subscribe_by_ids(self, var_ids: list[int], force: bool = False) -> dict:
        """Subscribe by numeric var_ids. Always includes CPU profiler variables.

        Enforces the cycle-time-derived safety cap unless force=True.
        """
    ...
    def _check_sub_size(self, requested: int, force: bool) -> dict | None:
        """Returns None if ok, or an error-dict suitable to return from an API.

        If `force=True`, never blocks (caller takes responsibility for any
        cycle-time violation that may follow).
        """
        if force:
            return None
        limits = self.get_subscription_limits()
        cap = limits["safe_subscription_limit"]
        if requested <= cap:
            return None
        return {
            "ok": False,
            "error": (
                f"Refusing to subscribe to {requested} variables: exceeds the "
                f"safe limit of {cap} for the current PLC cycle "
                f"({limits['cycle_time_ms']} ms). This protects the PLC from "
                f"a TCx maximum-cycle-time watchdog crash. Pass force=true "
                f"to override (only for controlled testing)."
            ),
            "limits": limits,
            "requested": requested,
        }
    ...
    def run_churn_test(self,
                       ...
                       churn_size: int = 1000,
                       churn_interval_s: float = 2.0,
                       churn_pool_start: int = 10000,
                       baseline_prefix: str = "gProtoTest",
                       force: bool = False) -> dict:
        """Real-world simulation:

        - Keep a permanent baseline subscribed for the entire test
          (baseline_prefix.* + CPU profiler vars). For your setup that's
          ~235 gProtoTest vars + ~10 CPU monitor vars.
        - Every churn_interval_s seconds, replace the "churn slot" with
          churn_size new random var_ids picked from the discovered
          available-vars pool starting at index churn_pool_start.
        - Each churn round is sent as a single SET command — that
          atomically unsubscribes the previous churn batch AND
          subscribes the new one in one wire message.
        - Throughout, sample the PLC CPU SysIdleRate + this-task LoadPct
          from the profiler vars (these are part of the permanent
          subscription so they update every cycle).

        Designed to be called from a background thread; progress is
        published to self._churn_progress for polling endpoints.

        Returns the final result dict. Also stored at self._churn_result.
        """
        import random

        if not self.connected:
            return {"ok": False, "error": "Not connected"}
        if not (self._discovery_done and self.available_vars):
            return {"ok": False, "error": "Run discovery first (POST /api/plc/discover)"}

        # Build the permanent (always-on) subscription.
        baseline_ids = sorted(
            vid for vid, info in self.registry.items()
            if vid in self.available_vars
            and info["name"].startswith(baseline_prefix + ".")
        )
        if not baseline_ids:
            return {
                "ok": False,
                "error": f"No vars found under prefix '{baseline_prefix}.*'",
            }
        cpu_ids = self._get_cpu_var_ids()
        permanent_ids = set(baseline_ids) | set(cpu_ids)

        # Build the churn candidate pool.
        available_sorted = sorted(self.available_vars)
        if churn_pool_start >= len(available_sorted):
            return {
                "ok": False,
                "error": (
                    f"churn_pool_start={churn_pool_start} is past the end of "
                    f"the discovered pool (size={len(available_sorted)})"
                ),
            }
        pool_candidates = available_sorted[churn_pool_start:]
        # Don't churn the permanent vars — keeps baseline truly steady.
        churn_pool = [vid for vid in pool_candidates if vid not in permanent_ids]
        if len(churn_pool) < churn_size:
            return {
                "ok": False,
                "error": (
                    f"Churn pool too small: {len(churn_pool)} candidates from "
                    f"index {churn_pool_start} (needed {churn_size})"
                ),
            }

        total_per_round = len(permanent_ids) + churn_size
        cap_err = self._check_sub_size(total_per_round, force)
        if cap_err is not None:
            return cap_err

        # Save current subscription so we can restore at the end.
        pre_subs = set(self.subscribed)

        # Init state for status endpoint.
        self._churn_running = True
        self._churn_done = False
        self._churn_result = None
        self._churn_progress = {
            "running": True,
            "round": 0,
            "total_rounds_estimate": int(duration_s / churn_interval_s),
            ...
            "churn_pool_size": len(churn_pool),
            "current_subs": 0,
            "cpu_idle_pct_latest": None,
            "cpu_load_pct_latest": None,
            "packet_rate_latest": 0.0,
            "drops_latest": 0,
        }

        cpu_idle_samples: list[float] = []
        ...
        # Reset live stats so the bench numbers reflect just this test.
        self.reset_stats()

        # First subscribe: permanent + initial churn batch.
        initial_churn = rng.sample(churn_pool, churn_size)
        initial_subs = list(permanent_ids | set(initial_churn))
        self.subscribed = set(initial_subs)
        self._send_subscribe_chunked(initial_subs)
        time.sleep(0.5)  # let PLC complete initial snapshot before counting

        # Start the throughput recorder; it'll auto-finalize when window passes.
        self.start_benchmark(duration_s + 1.0)

        next_round_at = start + churn_interval_s
        try:
            while self._churn_running:
                now = time.perf_counter()
                if now >= deadline:
                    break

                if now >= next_round_at:
                    new_churn = rng.sample(churn_pool, churn_size)
                    new_subs = list(permanent_ids | set(new_churn))
                    self.subscribed = set(new_subs)
                    self._send_subscribe_chunked(new_subs)
                    rounds_done += 1
                    next_round_at = now + churn_interval_s

                    # Sample CPU profiler (vars are in permanent set so live).
                    cpu = self.get_profiler_data()
                    idle = cpu.get("sys_idle_rate")
                    load = cpu.get("load_pct")
                    if idle is not None:
                        cpu_idle_samples.append(float(idle))
                    if load is not None:
                        cpu_load_samples.append(float(load))

                    # Publish progress for the UI.
                    elapsed = now - start
                    pkts = self.stats.get("packets_received", 0)
                    self._churn_progress.update({
                        "round": rounds_done,
                        "elapsed_s": round(elapsed, 1),
                        "current_subs": len(new_subs),
                        "cpu_idle_pct_latest": round(idle, 2) if idle is not None else None,
                        "cpu_load_pct_latest": round(load, 2) if load is not None else None,
                        "packet_rate_latest": round(pkts / max(0.1, elapsed), 1),
                        "drops_latest": self.stats.get("dropped_packets", 0),
                    })

                time.sleep(0.05)  # 20 Hz scheduler tick — cheap
        except Exception as e:
            print(f"[CHURN] Test loop error: {e}")

        # Wait for benchmark to finalize.
        while not self._bench_done and time.perf_counter() < deadline + 5.0:
            time.sleep(0.2)
        if not self._bench_done and self._bench_running:
            self._finalize_benchmark()

        bench = self._bench_result or {}
        elapsed = time.perf_counter() - start

        def stat_summary(samples: list[float]) -> dict | None:
            if not samples:
                return None
            s = sorted(samples)
            return {
        ...
        result = {
            ...
            "churn_size": churn_size,
            "churn_interval_s": churn_interval_s,
            "churn_pool_start": churn_pool_start,
            "churn_pool_size": len(churn_pool),
            "total_subs_per_round": total_per_round,
            "cpu_idle_pct": stat_summary(cpu_idle_samples),
            "cpu_load_pct": stat_summary(cpu_load_samples),
            # Throughput + integrity from the bench recorder.
            "packet_rate": bench.get("packet_rate"),
            "packets": bench.get("packets"),
            "var_updates": bench.get("var_updates"),
            "var_update_rate": bench.get("var_update_rate"),
            "dropped_packets": bench.get("dropped_packets"),
            "drop_pct": bench.get("drop_pct"),
            "avg_vars_per_pkt": bench.get("avg_vars_per_pkt"),
            "avg_bytes_per_pkt": bench.get("avg_bytes_per_pkt"),
            "inter_packet_ms": bench.get("inter_packet_ms"),
            "plc_interval_ms": bench.get("plc_interval_ms"),
            "grade": bench.get("grade"),
        }

        # Restore the user's previous subscription.
        if pre_subs:
            restore = sorted(pre_subs)
            self.subscribed = set(restore)
            self._send_subscribe_chunked(restore)
        else:
            self._send_tcp_command(0x01, [])  # explicit clear
            self.subscribed = set()
        time.sleep(0.5)
        self.reset_stats()
        ...
        self._churn_result = result
        self._churn_progress["running"] = False
        return result

    def stop_churn_test(self) -> dict:
        """Request the in-progress churn test to wind down at the next tick."""
        if not self._churn_running:
            return {"ok": False, "error": "No churn test running"}
        self._churn_running = False
        return {"ok": True, "stopping": True}

    def get_churn_status(self) -> dict:
        """Return progress (while running) or final result (when done)."""
        if self._churn_done and self._churn_result:
            return {"status": "done", "result": self._churn_result}
        if self._churn_running:
            return {"status": "running", "progress": self._churn_progress}
        return {"status": "idle"}
